...
if [ -d ${HOME_DIR}/${SERVER_NAME}/ ];then
    if [ ! -d ${BACKUP_DIR}/${TODAY}/${SERVER_NAME}/ ];then
        ...
        rm -rf ${HOME_DIR}/TimeJob/
    fi
fi


tar zxf ${TGZ_DIR}/IOTMP.CAS.TimeJob-2.0.0-prod.tgz -C $HOME_DIR
sudo chown -R iotmp:iotmp /home/iotmp/TimeJob/
sudo chmod 755 /home/iotmp/TimeJob/bin/*
...
echo '[Unit]
Description=TimeJob.service
[Service]
Type=forking
User=iotmp
ExecStart=/home/iotmp/TimeJob/bin/TimeJob.sh start
ExecStop=/home/iotmp/TimeJob/bin/TimeJob.sh stop
ExecReload=/home/iotmp/TimeJob/bin/TimeJob.sh restart
[Install]
WantedBy=multi-user.target'|sudo tee /usr/lib/systemd/system/TimeJob.service

sudo systemctl daemon-reload
sudo systemctl enable TimeJob
sudo systemctl start TimeJob
sudo systemctl status TimeJob
